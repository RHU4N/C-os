    ...
    <div className="container mt-5 row aling-items-center justify-content-center">
      ...
      <form className="card p-4 shadow-sm" onSubmit={handleSubmit} aria-label="Formulário de função">
        ...
        <div className="row g-2">
          <div className="col">
            <input type="number" className="form-control" placeholder="a" value={a} onChange={e => setA(e.target.value)} required aria-label="Coeficiente a" />
          </div>
          <div className="col">
            <input type="number" className="form-control" placeholder="b" value={b} onChange={e => setB(e.target.value)} required aria-label="Coeficiente b" />
          </div>
          {tipo === 'funcao2' && (
            <div className="col">
              <input type="number" className="form-control" placeholder="c" value={c} onChange={e => setC(e.target.value)} required aria-label="Coeficiente c" />
            </div>
          )}
          <div className="col">
            <input type="number" className="form-control" placeholder="x (opcional)" value={x} onChange={e => setX(e.target.value)} aria-label="Valor de x" />
          </div>
        </div>
        <button className="btn btn-primary mt-3" type="submit" disabled={loading} aria-busy={loading} aria-label="Calcular função">
